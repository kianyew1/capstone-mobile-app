...
void loop() {
  ...
  if (!mock_signal) {
    ...
    if (pending > 1){
      droppedSamplesThisSecond += (pending - 1);
      droppedSamplesTotal += (pending - 1);
    }
    pending = 0;
  } else {
  ...
  // SUS WHY IS THIS NOT JUST lastSendUs = nowUs
  ...
  if (isStreaming) {
    ...
    if (ringCount >= ECG_PACKET_SAMPLES) {
      ...
      // When is frameCount not 0 or ECG_PACKET_SAMPLES replace with bool?
      // When is it read that is signifcant
      ...
      if (sent) {
        ...
        packetizerConsume(); // Consider just putting the code of the funtion here?
