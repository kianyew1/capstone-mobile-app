...
def _analyze_ch2_capture(raw_ch2_20s: list[float]) -> dict[str, Any]:
    raw = np.asarray(raw_ch2_20s[:CAPTURE_TARGET_SAMPLES], dtype=float)
...
def _ingest_packet_hex(device_name: str | None, packets: list[str]) -> dict[str, Any]:
    ...
    with _state_lock:
        ...
        contact_detected = std_min <= contact_std <= std_max and abs_mean <= abs_mean_max
        print(
            "[CONTACT] "
